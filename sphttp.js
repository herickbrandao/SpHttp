...
var sphttp = {
	...
	async items(list, obj = {}) {
        ...
        if(typeof obj.select === "object") { obj.select = obj.select.join(); }
        if(typeof obj.expand === "object") { obj.expand = obj.expand.join(); }

		if(ID) {
			requestUrl += "(" + ID + ")";
            if(obj.versions) { requestUrl += "/versions"; }
            if(obj.select?.length) { requestUrl += concatenation + "$select=" + obj.select; concatenation = "&"; }
            if(obj.expand?.length>0) { requestUrl += concatenation + "$expand=" + obj.expand; concatenation = "&"; }
            if(obj.avoidcache) { requestUrl += concatenation+"$v=" + window.crypto.randomUUID(); concatenation = "&"; }
		...
		if(obj.select?.length) { requestUrl += concatenation + "$select=" + obj.select; }
        if(obj.expand?.length) { requestUrl += concatenation + "$expand=" + obj.expand; }
        if(obj.filter?.length) { requestUrl += concatenation + "$filter=" + obj.filter; }
        if(obj.avoidcache) { requestUrl += concatenation + "$v=" + window.crypto.randomUUID(); }

        if(!obj.recursive) {
            if(obj.orderby?.length) { requestUrl += concatenation + "$orderby=" + obj.orderby; }
            return this._rest(requestUrl,{},{ select: obj.select.split(",") });
        }
	...
	async add(list, obj) {
        ...
        return this._rest(url, { method: "POST", body: typeof obj === "object" ? JSON.stringify(obj) : obj });
	},
	...
	async attach(list, config = {}) {
        ...
        if(!window.FileReader) { throw('[SPHTTP] Your browser does not have support for FileReader!'); }
        if(isNaN(parseInt(ID))) { throw('[SPHTTP] List ID not found at attach request!'); }
        ...
        for(var i in items) { if(typeof items[i] === 'object') { appends.push(this._getFileBuffer(items[i])); } }
        ...
        return Promise.all(appends).then(function(promises) {
            ...
            for(var i in promises) {
                ...
                httpRes.push(this._fetch(url, { method: "POST", body: promises[i] }));
            }
	...
	async attachDoc(config = {}) {
        ...
        return Promise.all(appends).then(function(promises) {
            ...
            for(var i in promises) {
                ...
                httpRes.push(this._rest(url, { method: "POST", body: promises[i] }));
            }
	...
	async user(config = false) {
		...
		switch(typeof config) {
	        ...
	        case "number":
	            ...
	            break;
	        case "object":
	            if(config.ID) { return this._rest("_api/web/GetUserById(" + config.ID + ")/Groups"); }
	            break;
	        case "boolean":
	...
	async batch(batchRequests) {
		...
		var propData = [], actual = false;
		var changesetRequests = [], getRequests = [];
        ...
        .then(function(raw) {
        	var responses = [], parsedCtx = '', parsedRaw = raw.split("\r\n\r\n");
        		...
        		if(p.indexOf("No Content")>-1) {
        			...
        			try {
        				if(p.indexOf("<m:error")>-1) {
        					...
        					parsedCtx = { ok: false, data: p }; 
        				} else {
        					parsedCtx = { ok: true, data: p.split(">\r\n")[0] + ">" };
        				}
						
        			} catch(e) { parsedCtx = { ok: false, data: p }; }

					responses.push(parsedCtx);
        		} else if(p.indexOf("{")===0) {
        			try {
        				parsedCtx = JSON.parse(p.split("}\r\n")[0] + "}");
        				parsedCtx = { ok: true, data: parsedCtx }; 
        			} catch(e) { parsedCtx = { ok: false, data: p }; }
	...
	async _verifyDigest(refresh = false) {
		if(document.querySelector("#__REQUESTDIGEST") && document.querySelector("#__REQUESTDIGEST").value && !refresh) {
			...
			const _this = this;
			return this._rest("_api/contextinfo", { method: "POST" })
			.then(function(r) { _this.digest = r?.GetContextWebInformation?.FormDigestValue || ''; return r; })
			.catch(function(e) {
				_this._error(e);
				return e;
			});
	...
	async _fetch(url, fetchOptions) {
		...
		const abortFetch = setTimeout(function() { return controller.abort(); }, _this.timeout);
		const urlFetch = url;
        ...
        return new Promise(function(resolve, reject) {
        	...
        	var loop = async function(url) {
                _this._fetch(url).then(async function(resp) {
                    ...
                    if(resp?.d?.__next) { await loop(resp.d.__next); } else { resolve(content);}
                })
